...
// IsFresh — true, если snapshot достаточно свежий (раздел 6.3).
// Здесь только структурный флаг; порог MaxDataAgeMs проверяется вызывающим.
func (s *MarketSnapshot) StaleAge(now time.Time, maxAge time.Duration) bool {
