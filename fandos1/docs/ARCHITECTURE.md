...
```
cmd/
  server/   — точка входа HTTP/Telegram API                                [есть]
  worker/   — точка входа торгового движка                                 [есть]

webapp/     — Mini App frontend (Telegram WebApp)                          [есть]
deploy/     — конфигурация деплоя (docker-compose, systemd и т.п.)         [есть]

internal/
  app/          — сборка DI, wiring процессов                              [есть]
  ...
  telegram/     — bot backend, initData validation, Mini App API            [есть]
  auth/         — sessions, allowlist Admin IDs, replay-защита (13.5)       [есть]
  clocks/       — NTP/server-time sync, clock offset, recvWindow (раздел 24) [есть]
  lifecycle/    — graceful shutdown, DB-unavailable handling (раздел 28)    [есть]
  repository/   — PostgreSQL-persisted сущности                             [есть]
  outbox/       — transactional outbox                                      [есть]
  notifications/— Telegram-уведомления, redaction   [частично: telegram/notifier + таблица notifications]
  audit/        — immutable audit log                [есть: repository/audit.go + audit_log]
  observability/— Prometheus/OTel метрики, structured logs                  [есть]
```
