...
```
cmd/
  server/   — точка входа HTTP/Telegram API
  worker/   — точка входа торгового движка

internal/
  app/          — сборка DI, wiring процессов
  config/       — конфиг-модель (HOT/COLD категории, см. 15.3)
  domain/       — доменные типы: ExchangeID, Side, InstrumentType, состояния и т.д.
  decimal/      — двухконтурная арифметика (ADR-0002): Decimal (risk) + Fixed64 (hot)
  exchange/     — adapter.go (интерфейс) + реализации per-биржа
  marketdata/   — Level 2 all-market мониторинг, coalescing, backpressure
  orderbook/    — локальные стаканы, VWAP, sequence validation
  scanner/      — Level 3 кандидаты, первичные фильтры, ranking
  strategy/     — funding calendar, ExpectedNetPnL, candidate score
  allocation/   — распределение капитала, cross-pair contention (раздел 25)
  execution/    — coordinator, slices, ack-timeout, partial-fill repair, coordinated close
  risk/         — risk limits, margin, delta, ADL реакция, counterparty risk
  portfolio/    — агрегированный портфель, PnL, exposure
  rebalance/    — state machine, test/main barrier, circuit breaker
  credentials/  — envelope encryption, AEAD, в памяти только на подпись
  keyrotation/  — жизненный цикл ключей, kill switch, 2FA-gate мутаций (раздел 27)
  telegram/     — bot backend, initData validation, Mini App API
  auth/         — sessions, allowlist Admin IDs, replay-защита (13.5)
  clocks/       — NTP/server-time sync, clock offset, recvWindow (раздел 24)
  lifecycle/    — graceful shutdown, DB-unavailable handling (раздел 28)
  repository/   — PostgreSQL-persisted сущности
  outbox/       — transactional outbox
  notifications/— Telegram-уведомления, redaction
  audit/        — immutable audit log
  observability/— Prometheus/OTel метрики, structured logs
```
