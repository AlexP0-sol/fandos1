...
5. Скопировать .env.example → .env, заполнить DATABASE_URL и MASTER_KEY.
6. Создать БД, применить миграции (task "db: применить миграции").
7. F5 → выбрать "worker: DRY_RUN + демо-ордера" — движок пошёл. Готово.
```
